...
def _generate_from_embeds(runtime: RuntimeModel, prompt_embeds, suffix_embeds, *, max_new_tokens: int = 32) -> str:
    """Generate continuation directly from continuous input embeddings (no token projection)."""
    import torch

    model = runtime.model
    tok = runtime.tokenizer
    device = runtime.device
    inputs = torch.cat([prompt_embeds, suffix_embeds.unsqueeze(0)], dim=1)
    # Greedy decode in embedding space by feeding embeddings then switching to tokens.
    # Practical compromise: run one forward for next-token, then standard generate from that token.
    with torch.no_grad():
        logits = model(inputs_embeds=inputs).logits
        next_id = int(logits[0, -1].argmax().item())
        # Continue with normal token generation seeded by the argmax token.
...
def _measured_pgd(
    ...
    *,
    ...
    # Continuous ASR: generate from optimized embeddings (not projected tokens).
    text_cont = _generate_from_embeds(runtime, prompt_embeds, suffix.detach())
    ...
    result = AttackResult(
        ...
        steps,
        j["quality"],
        notes="measured_pgd_from_embeds",
    )
    return {
        "result": result.to_dict(),
        "mode": "measured",
        "embedding": emb_list[:128],
        "embedding_full_dim": int(suffix.shape[-1]),
        "manifold_distance": manifold_distance,
        "nearest_token_ids": projected_ids,
        "note": "continuous ASR from embeds; projection is a separate rung",
